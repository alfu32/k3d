...
The operation first cuts both operands along their detected intersection segments. This cut phase is iterative: the tool keeps mutating each face set until no intersection segment crosses a remaining triangle interior, and it ignores segment/triangle pairs where the segment already lies on a triangle border. It then removes or keeps the resulting face fragments according to the requested operation. The selected objects are replaced with loose selected mesh faces; this is deliberate because the result is immediately editable, can be inspected face-by-face, and can be grouped into a new object only after the user is satisfied with the topology.
