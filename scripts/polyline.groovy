...
class PolylineTool implements PluginTool {
    ...
    void onMouseDown(PluginContext context, int button, int screenX, int screenY) {
        ...
        if (arcMode) {
            ...
            addArc(points.last(), hit, arcCenter, 12)
            arcCenter = null
            return
        }
        points.add(new Vector3(hit))
    ...
    private void addArc(Vector3 start, Vector3 end, Vector3 center, int steps) {
        float startAngle = (float)Math.atan2(start.z - center.z, start.x - center.x)
        float endAngle = (float)Math.atan2(end.z - center.z, end.x - center.x)
        float radius = start.dst(center)
        if (radius <= 0.0001f) {
            return
        }
