...
class ExportersPlugin implements Plugin {
    ...
    private PluginCommand makeCommand(String id, String name, String description, String ext) {
        return [
            ...
            execute: { PluginContext context ->
                return showSaveDialog(context, name, ext)
            }
        ] as PluginCommand
    }
    ...
    private float[] transformTriangle(Matrix4 matrix, float[] tri) {
        ...
        for (int i = 0; i < 3; i++) {
            int base = i * 3
            Vector3 v = new Vector3(tri[base], tri[base + 1], tri[base + 2])
    ...
    private void writeStl(List<float[]> triangles, File file) {
        ...
        triangles.each { tri ->
            Vector3 a = new Vector3(tri[0], tri[1], tri[2])
            Vector3 b = new Vector3(tri[3], tri[4], tri[5])
            Vector3 c = new Vector3(tri[6], tri[7], tri[8])
    ...
    private PluginResult statusResult(String message, boolean success = true) {
        return new PluginResult(
            changes: [new PluginChange.StatusMessage(message)],
            success: success,
